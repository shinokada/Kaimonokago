...
class Welcome extends Shop_Controller {
    ...
    // moving to shop_controller
     ...
     private $module_name; // this will be grandparent directory name. welcome/controllers/welcome.php
     ...
     // not able to get router default_controller and assign this to index_path and module_name
  ...
  function  Welcome(){
    ...
    //$this->module_name = basename(dirname(dirname(__FILE__))); // this is the same as getting class name
  ...
  function index(){
        ...
        if ($this->input->post('lang')){
            ...
            redirect( $this->module_name.'/index','refresh');
        }
        //$webshop = $module;
	...
	function pages(){
		...
		if($path==$this->index_path){// for home page
			...
			redirect($this->module_name.'/contact','refresh');
		}elseif($path =='cart'){
		  	redirect($this->module_name.'/cart','refresh');
        }elseif($path =='playroom'){
		  	redirect('playroom/index','refresh');
		}elseif($path =='checkout'){
		 	redirect($this->module_name.'/checkout','refresh');
        //}elseif($path =='error'){
		 	//redirect($this->module_name.'/error','refresh');
		}else{
        ...
        if($this->session->userdata('lang')=='english' || $this->session->userdata('lang')==''){
			...
			$data['title'] = lang('webshop_shop_name')." | ".$page['name'];
					...
					redirect($this->module_name.'/error');
				}
  ...
  function contact(){
	...
	function message(){
		...
		else
		{
		    ...
		    redirect($this->module_name.'/contact');
		}  	
  	}  
		...
		if ($this->validation->run() == FALSE)
			...
			{	
				...
				if ($numrow == TRUE){
					...
					redirect( $this->module_name.'/login','refresh');
				}
			...
			redirect( $this->module_name.'/login');
			}
		...
		if ($this->input->post('name')){
			...
			$this->validation->set_fields($fields);
					...
					else
					{
						...
						redirect( $this->module_name.'/subscribe','refresh');
						}
			...
			if ($this->validation->run() == FALSE)
					{
						// if false outputs errors
						$this->validation->output_errors();
						redirect( $this->module_name.'/unsubscribe','refresh');
					}
					else
					{
						$email = $this->input->post('email');
	...
	function cart($productid=0){
		...
		if ($productid > 0){
			...
			redirect( $this->module_name.'/product/'.$productid, 'refresh');
		}else{
  ...
  function emailorder(){
		...
		if ($this->validation->run() == FALSE)
		{
			...
			redirect($this->module_name.'/ordersuccess');
		}
  	}
