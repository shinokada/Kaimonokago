    ...
    private $module;
  	...
  	function create(){
  		...
  		$this->bep_assets->load_asset_group('TINYMCE');
	   	...
	   	if ($this->input->post('name')){
			...
			// Set breadcrumb
			$this->bep_site->set_crumb($this->lang->line('kago_create'),'products/admin/create');
			$data['header'] = $this->lang->line('backendpro_access_control');
  	...
  	function edit($id=0){
	  	...
	  	if ($this->input->post('name')){
            ...
            // For other languages segment 4 is omc_products.product_id, product_id is id of english(original), omc_menu.id
            // for english is omc_products.id
            // $product_id is used to find translated languages and it is used to get info of english menu
            $product_id = $this->uri->segment(4);
            $data['translanguages'] =$this->MLangs->getTransLang($this->module,$product_id);
            $data['module']=$this->module;
			...
			// 	Set breadcrumb
			$this->bep_site->set_crumb($this->lang->line('kago_edit'),'products/admin/edit');
			$data['header'] = $this->lang->line('backendpro_access_control');
			$data['module'] = $this->module;
			$this->load->view($this->_container,$data);
		}
...
*/          //$data['category'] = $this->MCats->getCategory($cat_id);
            ...
            // Set breadcrumb
            $this->bep_site->set_crumb($this->lang->line('kago_add_translation').$selected_lang,'products/admin/edit/');
